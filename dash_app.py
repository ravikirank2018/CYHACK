...
# Create the Dash app with additional CSS for animations
app = dash.Dash(__name__, external_stylesheets=[dbc.themes.SOLAR, "https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css"])
...
def show_alerts(n):
    ...
    if n % 2 == 0:  # Simplified condition for testing
...
def update_attack_details(clickData):
    if clickData is None:
        return "Click on any point on the map to see the attack details."
    selected_lat = clickData['points'][0]['lat']
    selected_lon = clickData['points'][0]['lon']
    selected_attack = df[(df['latitude'] == selected_lat) & (df['longitude'] == selected_lon)]
    if selected_attack.empty:
        return "No details available for the selected attack."
    details = selected_attack.iloc[0]
